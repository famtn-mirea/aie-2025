...
def report(
    ...
    id_column: str = typer.Option(None, help="Колонка-идентификатор")
) -> None:
    """
    ...
    # 2. Качество в целом
    quality_flags = compute_quality_flags(summary, missing_df, id_column)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Количество столбцов с константными значениями: **{quality_flags['constant_columns_count']}**\n")
        f.write(f"- Количество столбцов с подозрительно большой или малой вариацией: **{quality_flags['abnormal_cv_count']}**\n\n")

        if id_column is None:
            f.write("Проверка уникальности id не проводилась\n\n")
        elif quality_flags['has_suspicious_id']:
            f.write(f"В столбце {id_column} обнаружены пустые или повторяющиеся значения\n\n")
        else:
            f.write(f"В столбце {id_column} не обнаружены пустые или повторяющиеся значения\n\n")
