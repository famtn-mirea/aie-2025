...
def compute_quality_flags(summary: DatasetSummary,
    ...
    - наличие константных колонок (с меньше чем 2 уникальными значениями);
    - наличие числовых колонок с CV меньше 0.1 или больше 0.5
    - Есть ли в колонке id неуникальные или пустые значениями
    """
    flags: Dict[str, Any] = {}
    ...
    flags["has_suspicious_id"] = False
    if id_column is not None:
        for col in summary.columns:
            if col.name == id_column and col.unique != col.non_null:
                flags["has_suspicious_id"] = True

    constant_columns_counter = 0
    for col in summary.columns:
        if col.unique < 2:
            constant_columns_counter += 1
    flags["constant_columns_count"] = constant_columns_counter
    flags["has_constant_columns"] = constant_columns_counter > 0

    abnormal_cv_counter = 0
    for col in summary.columns:
        if col.is_numeric and not np.isclose(col.mean, 0, 0.0001):
            cv = col.std / abs(col.mean)
            if cv < 0.1 or cv > 0.5:
                abnormal_cv_counter += 1

    flags["abnormal_cv_count"] = abnormal_cv_counter
    flags["has_abnormal_cv"] = abnormal_cv_counter > 0

    # Простейший «скор» качества
    score = 1.0
    score -= max_missing_share  # чем больше пропусков, тем хуже
    score -= 0.05 * constant_columns_counter
    score -= 0.02 * abnormal_cv_counter
    if flags["has_suspicious_id"]:
        score -= 0.2
